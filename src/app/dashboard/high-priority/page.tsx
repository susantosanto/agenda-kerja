...
export default function HighPriorityPage() {
  ...
  const fetchTasks = async () => {
    try {
      const res = await fetch("/api/tasks/filtered?filter=high-priority")
        ...
        <main className="flex-1 p-6 lg:pl-72">
          <div className="mx-auto max-w-6xl space-y-6">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>/</span>
              <span className="text-foreground">Prioritas Tinggi</span>
            </div>
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold tracking-tight">Prioritas Tinggi</h1>
                <p className="text-sm text-muted-foreground">Tugas dengan prioritas P1 dan P2</p>
              </div>
            </div>
            {tasks.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-12 text-center">
                <AlertTriangle className="h-12 w-12 text-muted-foreground" />
                <p className="mt-4 text-muted-foreground">Tidak ada tugas prioritas tinggi</p>
              </div>
            ) : (
