...
export default function OverduePage() {
  ...
  const fetchTasks = async () => {
    try {
      const res = await fetch("/api/tasks/filtered?filter=overdue")
        ...
        <main className="flex-1 p-6 lg:pl-72">
          <div className="mx-auto max-w-6xl space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold tracking-tight">Overdue</h1>
                <p className="text-sm text-muted-foreground">Tugas yang sudah jatuh tempo</p>
              </div>
            </div>
            {tasks.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-12 text-center">
                <AlertCircle className="h-12 w-12 text-muted-foreground" />
                <p className="mt-4 text-muted-foreground">Tidak ada tugas overdue</p>
              </div>
            ) : (
